module "cloud_auth_service_development" {
  ...
  container_image         = "088074866799.dkr.ecr.ap-southeast-1.amazonaws.com/cloud-auth-service:latest"
  port                    = 8080
  domain                  = "dev-cloud-auth-service.test.com"
  health_check_path       = "/"
}

module "cloud_auth_service_production" {
  ...
  container_image         = "088074866799.dkr.ecr.ap-southeast-1.amazonaws.com/cloud-auth-service:latest"
  port                    = 8080
  domain                  = "prod-auth-service.test.com"
  health_check_path       = "/"
}

output "cloud_auth_service_alb_endpoint_production" {
  description = "ALB Endpoint"
  value       = var.env == "Production" ? module.cloud_auth_service_production[0].alb_endpoint : null
}
